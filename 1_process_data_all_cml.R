...
dir.create(paste0(results, "_response/SHAP"))
...
hus = fread(paste0(export, "/mgg_samples_cml.csv"))

## Demo
demo1 = readRDS(paste0(export, "/demographics.rds")) %>% 
  dplyr::filter(referral_outside_of_uusimaa_for_allo == FALSE & referral_outside_of_uusimaa == FALSE &
                  blast_crisis_at_dg == FALSE & year(dg_date_combined) > 2006) %>%
...
cml_pat = fread("mounts/research/husdatalake/disease/processed_data/CML/CML_pathology.csv") %>%
  dplyr::filter(henkilotunnus %in% demo1$henkilotunnus) %>%
...
## Lab
lab = readRDS("mounts/research/husdatalake/disease/data/AML_CML/multilab.rds") %>%
  dplyr::filter(henkilotunnus %in% hus$henkilotunnus)
lab = lab %>%
  dplyr::mutate(tutkimus_lyhenne = gsub("\\(M\\)", "", gsub("\\(A\\)", "", tutkimus_lyhenne))) %>%
  dplyr::filter(tutkimus_lyhenne %in% c("B -Trom", "B -Eos", "B -Baso", "B -Lymf", "B -Neut", "B -Mono", "B -Leuk",
                                        "L -Neut", "L -Mono", "L -Eos", "B -Leuk", "L -Blast", "L -Baso", "L -Lymf")) %>%
  dplyr::left_join(hus %>% dplyr::select(henkilotunnus, sampletaken, samplenumber)) %>%
  dplyr::mutate(time_diff = abs(as.Date(sampletaken) - as.Date(naytteenotto_hetki))) %>%
  dplyr::arrange(time_diff) %>%
  group_by(henkilotunnus, samplenumber, tutkimus_lyhenne) %>%
  dplyr::slice(1) %>%
  ungroup() %>%
  group_by(henkilotunnus, samplenumber) %>%
  mutate(leuk = ifelse(tutkimus_lyhenne == "B -Leuk", tulos, NA)) %>%
  fill(leuk, .direction = "downup") %>%
  ungroup() %>%
  dplyr::mutate(tulos = as.numeric(tulos),
                leuk = as.numeric(leuk),
                tulos = ifelse(tutkimus_lyhenne %in% c("B -Eos", "B -Baso", "B -Lymf", "B -Mono", "B -Neut"), round(100*tulos/leuk), tulos),
                tutkimus_lyhenne = ifelse(tutkimus_lyhenne %in% c("B -Eos", "B -Baso", "B -Lymf", "L -Mono", "L -Neut"), gsub("B ", "L ", tutkimus_lyhenne), tutkimus_lyhenne)) %>%
  group_by(henkilotunnus, samplenumber, tutkimus_lyhenne) %>%
  summarise(tulos = max(tulos, na.rm = TRUE)) %>%
  ungroup() %>%
  ...
  reshape2::dcast(henkilotunnus+samplenumber~tutkimus_lyhenne, value.var = "tulos") %>%
  janitor::clean_names() %>%
...
mgg_images = readRDS("~/mounts/research/husdatalake/data/pathology/qpati/pathology_mgg.rds") %>%
  dplyr::inner_join(hus) %>%
  dplyr::left_join(demo %>% dplyr::select(henkilotunnus, dg_date_combined)) %>%
  dplyr::mutate(time_diff = as.numeric(as.Date(sampletaken) - as.Date(dg_date_combined))) %>%
  dplyr::select(henkilotunnus, samplenumber, sampletaken, statement, diagnose, mgg_time_diff = time_diff, album_id, database)
mgg1 = mgg %>%
  dplyr::filter(mgg_type_string == "blast") %>%
  dplyr::mutate(time_diff = as.numeric(as.Date(sampletaken) - as.Date(dg_date_combined)),
                mgg_section1 = ifelse(mgg_section == "bone_marrow", 1, ifelse(mgg_section == "summary", 2, 3))) %>%
  dplyr::arrange(henkilotunnus, abs(time_diff), mgg_section1) %>%
  group_by(henkilotunnus, samplenumber, sampletaken) %>%
  dplyr::slice(1) %>%
  ungroup() %>%
  dplyr::mutate(sampletaken = as.Date(sampletaken)) %>%
  dplyr::select(henkilotunnus, samplenumber, sampletaken, bm_blast_mgg = mgg_value_percentage)
## BM-PAD
pad = pathology %>%
  ...
  dplyr::group_by(henkilotunnus, sampletaken) %>%
  ...
  mutate(bm_blast_pad = ifelse(is.na(bm_blast_pad), 3, bm_blast_pad),
         sampletaken = as.Date(sampletaken)) %>%
  dplyr::select(henkilotunnus, sampletaken, bm_blast_pad)
bmblast = full_join(mgg1, pad) %>%
  dplyr::mutate(bm_blast = ifelse(!is.na(bm_blast_mgg), bm_blast_mgg, bm_blast_pad)) %>%
  dplyr::select(henkilotunnus, samplenumber, bm_blast) %>%
  dplyr::left_join(mgg_images)
...
hus1 = hus %>%
  ...
  distinct() %>%
  dplyr::select(-c(samplenumber, sampletaken, anamnesis, diagnose, statement, dg_date_combined))
...
abroad = abroad %>%
  ...
  dplyr::mutate(mgg_time_diff = 0,
                mgg_time_diff = as.numeric(mgg_time_diff))
hus1 = hus1 %>%
...
# Save
saveRDS(df, paste0(export, "_response/data_for_modelling_all_cml1.rds"))
fwrite(df, paste0(export, "_response/data_for_modelling_all_cml1.csv"))
